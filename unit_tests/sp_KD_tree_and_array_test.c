...
static bool TestKDASplitAndInit()
{
	...
	if(arrLeft[0] != 2 || arrLeft[1] != 3 || arrLeft[2]!=1 )
	{
	...
	for(;i<3;i++)
	{
		for(;j<3;j++)
		{
			...
			if(Splited[0]->indexOrdPerDim[i][j] != 2 && Splited[0]->indexOrdPerDim[i][j] != 3 && Splited[0]->indexOrdPerDim[i][j] != 1)
			{
				destroyArray(Splited[0]);
				destroyArray(Splited[1]);
				free(Splited);
		...
		for(;j<2;j++)
		{
			if(Splited[1]->indexOrdPerDim[i][j] != 4 && Splited[1]->indexOrdPerDim[i][j] != 0 )
			{
				destroyArray(Splited[0]);
				destroyArray(Splited[1]);
				free(Splited);
...
static bool TestKNN()
{
	...
	SPBPQueue* nearest3 = KNN(points,pnt,3,MAX_SPREAD,5);
	if(nearest3 == NULL)
	{
		spPointDestroy(pnt);
		destroyPointsArray(points,5);
